...
function replaceDownloadButton(button: HTMLElement) {
  ...
  newButton.style.cssText =
    "background-color: yellow; color: black; padding: 10px 20px; border-radius: 5px; text-decoration: none; display: inline-block; cursor: pointer;";
  ...
  newButton.addEventListener(
    ...
    async (e) => {
      ...
      const magnetLinks = Array.from(
        document.querySelectorAll<HTMLAnchorElement>("a.magnet-download")
      )
        .map((link) => link.href)
        .filter((href) => href);
...
function processButtons() {
  document
    .querySelectorAll<HTMLElement>(".torrent-modal-download")
    .forEach((btn) => {
